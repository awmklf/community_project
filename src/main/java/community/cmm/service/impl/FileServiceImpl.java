...
public class FileServiceImpl implements FileService {
	...
	public String uploadImage(FileVO vo) throws Exception {
		...
		// 허용되지 않은 이미지 타입인 경우 업로드 불가 반환
		if (!StringUtils.hasText(vo.getFileExtsn())) {
			return "NOTALLOW_" + vo.getOrignlFileNm();
		}
		...
		fileInfo += "&sFileName=" + vo.getOrignlFileNm() + "." + vo.getFileExtsn();
	...
	public FileVO thumbnail(FileVO vo) throws Exception {
		...
		log.info("imageThumbnail : {}", imageThumbnail);
		...
		log.info("readImage : {}", readImage);
		int width = readImage.getWidth();
		int height = readImage.getHeight();

		if (width <= 600 && height <= 600) {
			log.info("원본 이미지 크기 600x600 미만");
			Thumbnails.of(originalImage).size(120, 120).keepAspectRatio(true).toFile(new File(imageThumbnail));

		} else {
			log.info("원본 이미지 크기 600x600 이상");
			Thumbnails.of(originalImage).sourceRegion(Positions.CENTER, 600, 600).size(120, 120).keepAspectRatio(true).toFile(new File(imageThumbnail));
		}
