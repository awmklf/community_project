...
/*function getCsrfToken() {
	...
	xhr.onreadystatechange = function() {
		if (xhr.readyState === 4 && xhr.status === 200) {
			...
			csrfHeader = response.headerName;
...
// 24.09.13 - 스프링 시큐리티 토큰을 가져오기 위한 함수 fetch로 변경
fetch('/csrf-token')
...
var rFilter = /^(image\/bmp|image\/gif|image\/jpg|image\/jpeg|image\/png|image\/webp)$/i; // 09.12 이미지 타입 추가
var rFilter2 = /^(bmp|gif|jpg|jpeg|png|webp)$/i;  // 09.12 WebP 추가
var nTotalSize = 0;
...
function checkDragAndDropAPI() {
	try {
		if (!oNavigator.ie) {
			...
			} else if (oNavigator.mobile || oNavigator.msafari || oNavigator.mopera || oNavigator.mie) { //모바일인경우 드래그앤드랍 지원안하도록 수정 09.12
				bSupportDragAndDropAPI = false;
			} else {
...
function callAjaxForHTML5(tempFile, sUploadURL) {
	return new Promise((resolve, reject) => {
		...
		oAjax.header(csrfHeader, csrfToken); // CSRF 토큰 추가
...
function makeArrayFromString(sResString) {
	...
	console.log('makeArrayFromString 실행');
	
	try {
		if (!sResString || sResString.indexOf("sFileURL") < 0) {
			return;
		}
		console.log('sFileURL 검문 통과');
		...
		console.log('htTemp[aSubTemp[0]] :', htTemp[aSubTemp[0]]);
	} catch (e) {
		console.log(e);
	}
	...
	console.log("렝스, 카운트 비교 : ", aResult.length, nImageFileCount);
...
function uploadImage(e) {
	html5Upload(); // 09.14 싱글, 멀티 파일 업로드 진행 통일
	/*if (!bSupportDragAndDropAPI) {
		console.log('확인클릭 제너럴 업로드 실행');
		generalUpload();
	} else {
		html5Upload();
	}*/
...
/**
 * 09.14 싱글 이미지 업로드 함수 구현
 */
function uploadSingleImage(file) {
    var formData = new FormData();
    formData.append("file", file);
	
    fetch('/board/uploadSingleImage', {
        method: 'POST',
        headers: {
    ...
    .then(data => {
		if(data.e) {
			...
			nImageFileCount = 1; // 싱글, 멀티 업로드 통합 후 싱글 업로드를 위한 사진 수 셋
			makeArrayFromString(data.responseText);
		}
    })
    .catch(error => {
...
* 09.14 uploadSingleImage 함수로 업로드 로직 대체
*/
function callFileUploader() {
	oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"), {
		sUrl: location.href.replace(/\/[^\/]*$/, '') + '/file_uploader.php',	//샘플 URL입니다.
		sCallback: location.href.replace(/\/[^\/]*$/, '') + '/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
		sFiletype: "*.jpg;*.png;*.bmp;*.gif;*.webp",						//허용할 파일의 형식. ex) "*", "*.*", "*.jpg", 구분자(;)	
		...
		select: function(oCustomEvent) {
			...
			if (oCustomEvent.bAllowed === true) {
				...
				goStartMode();
                var file = document.getElementById('uploadInputBox').files[0];
				uploadSingleImage(file); // 09.14 기존 싱글 업로드 대체 로직 추가
			} else {
				goReadyMode();
				oFileUploader.reset();
			}
